...
import { getTeams, getEmployees, getRemoteRequests, decideRemote } from '@/lib/backend';

export const dynamic = 'force-dynamic';

// POST: a team leader decides a remote-attendance request for one of THEIR team
// members. Authorization is enforced here: the caller must lead the team of the
...
export async function POST(request, ctx) {
  ...
  try {
    const remoteData = await getRemoteRequests({}, user.orgId);
    const req = (remoteData.requests || []).find((r) => String(r.id) === String(id));
    ...
    const result = await decideRemote(id, decision, note, user.orgId, user.id);
