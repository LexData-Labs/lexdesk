...
import { getTeams, getEmployees, getRemoteRequests } from '@/lib/backend';

export const dynamic = 'force-dynamic';

// GET: remote-attendance requests for members of the team(s) the caller LEADS.
// The caller is an employee; leadership is resolved + enforced here (the API key
// is org-wide).
export async function GET(request) {
  ...
  try {
    ...
    const remoteData = await getRemoteRequests(status ? { status } : {}, user.orgId);
    const requests = (remoteData.requests || []).filter((r) => memberUids.has(String(r.uid)));
