...
import { getAccessories, createAccessory } from '@/lib/backend';
...
export async function GET(request) {
  ...
  try {
    return NextResponse.json(await getAccessories(user.orgId));
  } catch (err) {
...
export async function POST(request) {
  ...
  try {
    const result = await createAccessory(body, user.orgId);
