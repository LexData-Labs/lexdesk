...
export default function TeamApprovalsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/team/leave${qs}`, {
  ...
  const decide = async (id, decision) => {
    ...
    if (decision === 'rejected') {
      ...
      if (r === null) return; // Cancel aborts the rejection.
      note = r;
    }
    setBusyId(id);
    setError('');
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`/api/team/leave/${encodeURIComponent(id)}`, {
        method: 'POST',
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        ...
        subtitle="Review and decide leave requests from your team"
      ...
      {!loading && !isLeader ? (
        <div className="card text-[var(--color-text-muted)] text-sm text-center py-12">
          You don’t lead a team yet. When an admin makes you a team leader, your team’s leave requests appear here.
        </div>
      ) : (
        <>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  ...
                  {!loading && list.length === 0 && (
                    <tr><td colSpan={6} className="py-8 text-center text-[var(--color-text-muted)]">No {status === 'all' ? '' : status} requests for your team.</td></tr>
                  )}
                  {loading && (
