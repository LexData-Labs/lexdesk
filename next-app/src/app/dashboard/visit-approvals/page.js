...
const fmtRange = (a, b) => (!a ? '—' : a === b ? a : `${a} → ${b}`);
...
export default function VisitApprovalsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/visit${qs}`, { headers: { Authorization: `Bearer ${token}` }, cache: 'no-store' });
  ...
  const decide = async (id, decision) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`/api/admin/visit/${encodeURIComponent(id)}`, {
        method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ decision, note }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || `HTTP ${res.status}`);
      setFeedback(`Visit ${decision}.`); setTimeout(() => setFeedback(''), 3000);
      await load();
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader title="Visit Approvals" subtitle="Review field-visit applications" actions={<button onClick={load} className="btn-outline py-2 px-4 text-sm">Refresh</button>} />
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="text-left text-[var(--color-text-muted)] text-xs border-b border-[var(--color-card-border)]">
              <th className="py-3 px-4 font-medium">Employee</th><th className="py-3 px-4 font-medium">Dates</th><th className="py-3 px-4 font-medium">Place</th><th className="py-3 px-4 font-medium">Purpose</th><th className="py-3 px-4 font-medium">Status</th><th className="py-3 px-4 font-medium text-right">Action</th>
            </tr></thead>
            <tbody>
              {list.map((r) => (
                <tr key={r.id} className="border-t border-[var(--color-card-border)] hover:bg-white/[0.02] align-top">
                  ...
                  <td className="py-3 px-4 text-[var(--color-text-main)] max-w-[200px]">{r.place || '—'}</td>
                  <td className="py-3 px-4 text-[var(--color-text-muted)] max-w-[220px]">{r.subject || '—'}</td>
              ...
              {!loading && list.length === 0 && <tr><td colSpan={6} className="py-8 text-center text-[var(--color-text-muted)]">No {status === 'all' ? '' : status} visits.</td></tr>}
